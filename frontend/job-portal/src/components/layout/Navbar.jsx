...
import { Bookmark, Briefcase, Building2, Download, Edit3, Mail, X } from "lucide-react";
          ...
          <div className="flex items-center space-x-3">
            ...
            {isAuthenticated ? (
              <ProfileDropdown
                ...
                avatar={user?.avatar || ''}
                companyName={user?.name || user?.companyName || 'User'}
                email={user?.email || ''}
                userRole={user?.role || 'jobseeker'} // Fixed: should be 'role', not 'userRole'
                onLogout={logout}
              />
            ) : (
              <>
